                           ...
                           const struct hmap *lr_datapaths)
{
    ...
    /* NAT, route policies, LBs and dynamic routing on this router pull in
     * stateful/routable/advertised-route/policy dependencies that live
     * outside the port's lflow_ref and are not tracked here. */
    const struct nbrec_logical_router *nbr = od->nbr;
    if (nbr->n_nat || nbr->n_policies ||
    ...
    if (lrp_affects_static_routes(lr_datapaths, od, lrp_name, networks)) {
        return true;
    }
    ...
    const struct nbrec_static_mac_binding_table *nb_smb_table)
{
    return lrp_port_needs_recompute(od, nbrp, ls_ports, nb_smb_table)
           || lrp_router_needs_recompute(od, nbrp->name, networks,
                                         lr_datapaths);
}
