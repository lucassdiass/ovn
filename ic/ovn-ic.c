              ...
              const char *route_table, const struct uuid *ic_route_uuid)
{
    ...
    basis = hash_bytes(prefix, sizeof *prefix, (uint32_t)plen);
              ...
              const struct uuid *ic_route_uuid, uint32_t hash)
{
    ...
    HMAP_FOR_EACH_WITH_HASH (r, node, hash, routes) {
        if (ipv6_addr_equals(&r->prefix, prefix) &&
            ...
            (!ic_route_uuid || uuid_equals(&r->ic_route_uuid, ic_route_uuid))) {
            return r;
        }
    }
    return NULL;
                       ...
                       const struct nbrec_logical_router_port *ts_lrp)
{
    ...
    for (int i = 0; i < lr->n_static_routes; i++) {
        ...
        if (smap_get_uuid(&nb_route->external_ids, "ic-learned-route",
            ...
            if (!add_to_routes_learned(&ic_lr->routes_learned, nb_route, lr, &isb_uuid)) {
                static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
